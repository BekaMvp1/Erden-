...
export default function FloorTasks() {
  ...
  const groups = React.useMemo(() => {
    ...
    tasks.forEach((t) => {
      ...
      if (!byKey[key]) {
        byKey[key] = { key, order_id: t.order_id, floor_id: fid, floor_name: t.floor_name || '—', order_tz_model: t.order_tz_model || t.order_title, tasks: [] };
      }
                      ...
                      return (
                        <tr key={task.id} className="border-b border-white/10 dark:border-white/10">
                          <td className="px-4 py-2 text-[#ECECEC] dark:text-dark-text">
                            <span className="font-medium">{task.order_tz_model || task.order_title}</span>
                          </td>
