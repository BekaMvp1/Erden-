...
/**
 * GET /api/sewing/plan-dates?order_id=&floor_id=&date_from=&date_to=
 * Даты плана по заказу и этажу. Опционально date_from, date_to — ограничить период (как в карточке Пошив).
 */
router.get('/plan-dates', async (req, res, next) => {
  try {
    ...
    const date_from = req.query.date_from ? String(req.query.date_from).slice(0, 10) : null;
    const date_to = req.query.date_to ? String(req.query.date_to).slice(0, 10) : null;
    ...
    const planWhere = { order_id, floor_id };
    if (date_from && date_to) planWhere.date = { [Op.between]: [date_from, date_to] };
...
router.get('/board', async (req, res, next) => {
  try {
    ...
    // Факт по дням: sewing_fact + fallback на production_plan_day.actual_qty
    // (ввод в Планировании идёт в production_plan_day, Пошив должен видеть эти данные)
    const factRows = await db.SewingFact.findAll({
      where: {
        floor_id: { [Op.in]: sewingFloorIds },
        date: { [Op.between]: [date_from, date_to] },
    ...
    // Fallback: если sewing_fact пуст — взять actual_qty из production_plan_day (ввод в Планировании)
    const ppdRowsRaw = await db.ProductionPlanDay.findAll({
      where: {
        floor_id: { [Op.in]: sewingFloorIds },
        date: { [Op.between]: [date_from, date_to] },
      },
      attributes: ['order_id', 'floor_id', 'date', 'actual_qty'],
      raw: true,
    });
    const ppdRows = (ppdRowsRaw || []).filter((r) => (Number(r.actual_qty) || 0) > 0);
    (ppdRows || []).forEach((r) => {
      const k = `${r.order_id}-${r.floor_id}`;
      const dateStr = r.date ? String(r.date).slice(0, 10) : null;
      if (!dateStr || !planByKey[k]) return;
      const existing = (factByKey[k] || []).find((x) => x.date === dateStr);
      if (!existing && Number(r.actual_qty) > 0) {
        if (!factByKey[k]) factByKey[k] = [];
        factByKey[k].push({ date: dateStr, fact_qty: Number(r.actual_qty) });
      }
    });
    ...
    const cutByKey = {};
    for (const t of cuttingDone) {
      const k = `${t.order_id}-${t.floor}`;
      if (!cutByKey[k]) cutByKey[k] = 0;
      const variants = t.actual_variants && Array.isArray(t.actual_variants) ? t.actual_variants : [];
      variants.forEach((v) => { cutByKey[k] += Math.max(0, parseInt(v.quantity_actual, 10) || 0); });
    }
    const alreadySewnByKey = {};
    if (orderIds.length > 0) {
      const sewnAggRows = await db.sequelize.query(
        `SELECT sb.order_id, sb.floor_id, COALESCE(SUM(sbi.fact_qty), 0)::int AS already_sewn
         FROM sewing_batches sb
         JOIN sewing_batch_items sbi ON sbi.batch_id = sb.id
         WHERE sb.order_id IN (:orderIds) AND sb.floor_id IN (:floorIds)
         GROUP BY sb.order_id, sb.floor_id`,
        {
          replacements: { orderIds, floorIds: sewingFloorIds },
          type: db.sequelize.QueryTypes.SELECT,
        }
      );
      (sewnAggRows || []).forEach((r) => {
        alreadySewnByKey[`${r.order_id}-${r.floor_id}`] = Number(r.already_sewn) || 0;
      });
    }
    ...
    for (const key of keys) {
      ...
      const cut_total = cutByKey[key] || 0;
      const already_sewn = alreadySewnByKey[key] || 0;
      const available_for_sewing = Math.max(0, cut_total - already_sewn);
 ...
 * Проверка: завершён ли пошив — партия создана (READY_FOR_QC или QC_DONE).
 * Исторически искали DONE; новая логика: партии создаются сразу со статусом READY_FOR_QC.
 */
router.get('/complete-status', async (req, res, next) => {
  try {
    ...
    const where = {
      order_id,
      status: { [Op.in]: ['DONE', 'READY_FOR_QC', 'QC_DONE'] },
    };
...
router.post('/complete', async (req, res, next) => {
  try {
    ...
    let [factAgg] = await db.sequelize.query(
    ...
    let fact_sum = Number(factAgg?.[0]?.fact_sum) || 0;

    // Fallback: страница «Задачи по этажам» сохраняет факт в production_plan_day; sewing_fact пуст.
    // Синхронизируем из production_plan_day и используем для создания партии.
    if (fact_sum <= 0) {
      const [ppdAgg] = await db.sequelize.query(
        `SELECT date::text AS date, COALESCE(actual_qty, 0)::int AS fact_qty
      ...
      if (ppdAgg && ppdAgg.length > 0) {
        const ppdSum = ppdAgg.reduce((s, r) => s + (Number(r.fact_qty) || 0), 0);
        if (ppdSum > 0) {
          for (const r of ppdAgg) {
            await db.SewingFact.upsert(
              {
                order_id: Number(order_id),
                floor_id: effectiveFloorId,
    ...
    // Ограничение: если есть раскрой по заказу/этажу — пошив не может превысить фактически раскроенное
    const cuttingTasks = await db.CuttingTask.findAll({
      where: { order_id: Number(order_id), floor: effectiveFloorId },
      attributes: ['status', 'actual_variants'],
      raw: true,
    });
    let cutTotal = 0;
    (cuttingTasks || []).filter((t) => t.status === 'Готово').forEach((t) => {
      const variants = t.actual_variants && Array.isArray(t.actual_variants) ? t.actual_variants : [];
      variants.forEach((v) => { cutTotal += Math.max(0, parseInt(v.quantity_actual, 10) || 0); });
    });
    if (cuttingTasks.length > 0 && cutTotal === 0) {
      return res.status(400).json({
        error: 'Сначала завершите раскрой (кнопка «Завершить по факту»). В пошив можно передавать только фактически раскроенное количество.',
      });
    }
    if (cutTotal > 0) {
      const [sewnAgg] = await db.sequelize.query(
        `SELECT COALESCE(SUM(sbi.fact_qty), 0)::int AS already_sewn
         FROM sewing_batches sb
         JOIN sewing_batch_items sbi ON sbi.batch_id = sb.id
         WHERE sb.order_id = :order_id AND sb.floor_id = :floor_id`,
        { replacements: { order_id: Number(order_id), floor_id: effectiveFloorId } }
      );
      const alreadySewn = Number(sewnAgg?.[0]?.already_sewn) || 0;
      const availableForSewing = Math.max(0, cutTotal - alreadySewn);
      if (fact_sum > availableForSewing) {
        return res.status(400).json({
          error: `Нельзя передать в пошив ${fact_sum} шт. Доступно по раскрою: ${availableForSewing} (раскроено ${cutTotal}, уже в пошиве ${alreadySewn}).`,
        });
      }
    }
    ...
    if (plan_sum > 0 && fact_sum > plan_sum) {
      return res.status(400).json({
        error: `Факт пошива (${fact_sum}) не может превышать план (${plan_sum}) за выбранный период.`,
      });
    }
